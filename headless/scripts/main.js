...
'use strict'; {
    ...
    window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
            ...
            if (this.IsWebView2Wrapper()) self["chrome"]["webview"].addEventListener("message", e => this._OnWrapperMessage(e.data));
        ...
        async _Init(opts) {
            if (this._exportType ===
                "macos-wkwebview") this._SendWrapperMessage({
                "type": "ready"
            });
...
'use strict'; {
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        _OnVisibilityChange(isHidden) {
            if (this._pageVisibilityIsHidden ===
                isHidden) return;
...
'use strict'; {
    ...
    const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
        ...
        _OnNavigate(e) {
            ...
            if (type === "back")
                ...
                if (self["cordova"] && self["cordova"]["InAppBrowser"]) self["cordova"]["InAppBrowser"]["open"](url,
                    "_system");
                else if (exportType === "preview" || this._iRuntime.IsWebView2Wrapper()) window.open(url, "_blank");
        ...
        _OnExitFullscreen() {
            if (this._iRuntime.IsWebView2Wrapper() || this._exportType === "macos-wkwebview") {
    ...
    self.RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)
    window.dispatchEvent(new CustomEvent('basket-ready'));
};